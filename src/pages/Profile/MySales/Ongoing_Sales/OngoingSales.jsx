...
const OngoingSales = ({activeTab, setActiveTab}) => {
 ...
 id: 1,
  ...
  return (
     <>
      {activeTab=='ongoing' && order && Object.keys(order).length !== 0 &&  
        <div className="grid [grid-template-rows:repeat(auto-fit,_minmax(min-content,_1fr))] gap-10">
            <SingleOngoingSale order={order}/>
            <SingleOngoingSale order={order} />
            <SingleOngoingSale order={order}/>
        </div> 
      } 
      {
        activeTab=='ongoing' && (order==null || Object.keys(order).length === 0) && 
        <NoSales >
          <h2>You haven’t any ongoing sales now</h2>
        </NoSales>
      }
    </>
