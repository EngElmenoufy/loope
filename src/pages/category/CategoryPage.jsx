...
function CategoryPage({ categories, brands, products, addOrRemoveFavorite }) {
  ...
  const applyFilters = (initalFilter) => {
    if (initalFilter) {
      setAppliedFilters({ ...initalFilter });
      filterProducts(initalFilter);
      setIsFilterOpen(false);
    } else {
  ...
  const filterProducts = (currentFilters) => {
    const filtered = products.filter((product) => {
      ...
      if (currentFilters.hasDiscount !== "all") {
        ...
        if (currentFilters.hasDiscount === "yes" && !hasDiscount) {
          return false;
        }

        if (currentFilters.hasDiscount === "no" && hasDiscount) {
          return false;
        }
      }
